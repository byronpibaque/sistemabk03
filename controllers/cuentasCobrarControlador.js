...
export default {
  ...
  listxfiltro: async (req, res, next) => {
    try {
      let filtroArray = req.body.filtro;

      const creafiltro = {
        $and: [],
      };
      if (req.body.estado){
        creafiltro.$and.push({ estado: req.body.estado });
      }

      if (req.body.finicio && req.body.ffin) {
        creafiltro.$and.push({
          createdAt: { $gte: req.body.finicio, $lt: req.body.ffin },
        });
      }
      ...
      await ctacobrar
          ...
          .exec(async function (err, CtaCobrar) {
            if (err)
            ...
            if (CtaCobrar) {
              let lista = [];
              if (filtroArray) {
                for (const element of CtaCobrar) {
                  const cupo = await models.Cupo.findOne({
                    codigoPersona: element.codigoPersona._id,
                  });
                  filtroArray.forEach((element2) => {
                    if (element2.value === cupo.descripcion) {
                      lista.push(element);
                    }
                  });
                }
  ...
  list: async (req, res, next) => {
    try {
      ...
      ctacobrar
        .find({
          $and: [{ estado: 1 }],
          $or: [{ numComprobante: new RegExp(valor, "i") }],
        })
        .populate([
          { path: "codigoVenta", model: "ventas" },
          { path: "codigoUsuario", model: "usuarios", select: "nombres" },
  ...
  listOne: async (req, res, next) => {
    try {
      ...
      ctacobrar
        .find({ $and: [{ descripcion: valor }, { numComprobante: valor2 }] })
        .populate([
          { path: "codigoVenta", model: "ventas" },
          { path: "codigoUsuario", model: "usuarios", select: "nombres" },
          { path: "codigoPersona", model: "persona", select: "nombres" },
  ...
  activate: async (req, res, next) => {
    try {
      const reg = await ctacobrar.findByIdAndUpdate(
        { _id: req.body._id },
        { estado: 1 }
      );
      res.status(200).json(reg);
  ...
  listporFechasAdmin: async (req, res, next) => {
    try {
      ...
      let ffin = req.query.fechafin;
      ctacobrar
        .find({ $and: [{ createdAt: { $gte: finicio, $lt: ffin } }] })
        .populate([
          {
            path: "codigoTipoComprobante",
  ...
  reporteCuentas: async (req, res, next) => {
    try {
      ...
      await models.Persona.populate(reg, {
        path: "_id.codigoPersona",
        select: { nombres: 1 },
      });
