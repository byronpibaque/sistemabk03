...
export default {
  ...
  list: async (req, res, next) => {
    try {
      ...
      if (codigoFarmacia != null) {
        const reg = await models.Arqueos.find({
          $and: [
  ...
  remove: async (req, res, next) => {
    try {
      const reg = await models.Arqueos.findByIdAndDelete({
        _id: req.query._id,
      });
      res.status(200).json(reg);
    } catch (e) {
  ...
  activate: async (req, res, next) => {
    try {
      const reg = await models.Arqueos.findByIdAndUpdate(
        { _id: req.body._id },
        { estado: 1 }
      );
      res.status(200).json(reg);
    ...
    try {
      ...
      Venta.find({
        ...
        .exec(function (err, Venta) {
          if (err)
            throw res.status(500).send({ message: "Ocurrió un error: " + err });
          if (Venta) {
  ...
  listporFechasAdmin: async (req, res, next) => {
    try {
      ...
      models.Arqueos.find({
        $and: [
          { createdAt: { $gte: new Date(finicio + "T12:00:00.000Z") } },
